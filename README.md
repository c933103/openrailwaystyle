...
The upstream vector service excludes all non-operating lines at zoom 7, disused lines before zoom 11, and abandoned/razed lines before zoom 12. A separate GeoJSON layer fills those gaps from zoom 7 to 12 using [VK Maps Overpass](https://maps.mail.ru/osm/tools/overpass/), which permits small public projects. It requests only the current regional viewport, never a world extract or prefetched tiles. Requests are debounced, serialized, spaced at least five seconds apart, and cached for four recent regions in memory. Failures trigger a 30-second cooldown and a visible coverage message. Views wider than 18° or taller than 12° ask for another zoom step instead of silently truncating coverage. The standard vector layer remains available if the regional service fails. The supplementary layer draws only lifecycle/feature classes omitted by the vector source at each zoom, and hands off fully to the vector source at zoom 12. Geometry gaps from clipped Overpass output are kept separate so the map does not invent connecting segments.
