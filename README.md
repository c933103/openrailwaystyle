...
- A compass that resets north, metric or imperial units, a “more detail” view (the next zoom level drawn at half size, so the same area shows more tiles and features), and drawing tools for points, lines and areas with length and area, kept in the browser and saved or opened as GeoJSON.
