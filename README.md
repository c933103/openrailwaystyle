...
One shared language selector controls map, station and railway labels. Western languages fall back to English and recorded Latin names before native names. Russian prefers recorded Cyrillic names, then English. Chinese tries Chinese variants and available ideographic names, including ordinary Japanese, Korean Hanja (`name:ko-Hani`) and Vietnamese Chữ Nôm (`name:vi-Hani`), before English; a Japanese name recorded as “kana (kanji)” shows only the kanji. Japanese prioritizes ideographic names over kana-only names. These Han-character fallbacks apply only within China, Taiwan, Hong Kong, Macau, Japan, the Koreas and Vietnam; elsewhere Chinese and Japanese use only names recorded in that language, then English, then the native name, so no place outside that region is given a borrowed Han name. The region comes from Natural Earth 1:10m de facto boundaries (`scripts/build-cjkv-region.mjs`); coasts are simplified to about 1 km with an 8 km tolerance for piers and reclaimed land, and land borders keep full detail. Unicode script checks include supplementary-plane Han characters; names are never automatically translated, transliterated or converted between character standards. If no preferred name exists, the native name remains visible.
